...
def heuristic(cx, cy, endX, endY):
    d1 = abs(cx - endX)
    d2 = abs(cy - endY)
    return (d1 + d2) + (math.sqrt(2) - 2) * min(d1, d2)
...
def AStar(startX, startY, endX, endY, speed):
    ...
    while len(openQueue) > 0:
        ...
        for i in adjacent:
            #get the parents gScore
            tempGScore = (gScore.get((currentPixel[1][0], currentPixel[1][1]))) + 1
            #if this gScore is LESS than the one an adjacent pixel already has, update that pixels gScore to tempGScore, fScore to tempGScore + its heuristic, and update its parent to the curentPixel.
